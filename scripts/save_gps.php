...
$id = isset($_POST['vh_id']) ? intval($_POST['vh_id']) : 0; //755
$mode = isset($_POST['vh_mode']) ? $mysqli->real_escape_string($_POST['vh_mode']) : ''; //edit
...
$yahooid = isset($_POST['yahooid']) ? $mysqli->real_escape_string($_POST['yahooid']) : '';
$response = array();
$response['code'] = 'ERROR';
$response['msg'] = '';
$response['mode'] = $mode;
...
switch ($mode) {
        ...
        if (((int) $user_id <= 0) || (trim($imei) == '') || (trim($nopol) == '')) {
            ...
            if ($result) {
                if ($result->num_rows <= 0) {
                    $sql = "INSERT INTO vehicle (`nopol`,`user_id`,`icon_map`,`icon`,`imei`,`phone`,`timezone`,`vh_brand`,`gps_brand`,`guaranty_date`,`install_date`,`drv_name`,`drv_phone`,`descr`,`yahooid`,`center_number`,`max_park`) ";
                    $sql .=" VALUES(";
                    $sql .="'" . $nopol . "','" . $user_id . "',";
    ...
    case 'edit':
        ...
        $sql .="gps_brand='" . $gps_brand . "',guaranty_date='" . $guaranty . "',";
        $sql .="install_date='" . $install_date . "',";
        $sql .="drv_name='" . $drv_name . "',drv_phone='" . $drv_phone . "',";
        $sql .="descr='" . $descr . "',yahooid='" . $yahooid . "',center_number='" . $center_number . "' WHERE id='" . $id . "'";
        $response['data'] = $sql;
        if ($mysqli->query($sql)) {
            $response['code'] = 'SUCCESS';
