...
switch ($mode) {
    ...
    case 'edit':
        ...
        switch ($user_level) {
            case 'admin':
                $sql = "UPDATE user SET reseller_id='{$reseller_id}',login='{$login}',real_name='{$real_name}',user_access='{$user_access}',address='{$address}',phone='{$phone}',email='{$email}',description='{$description}',level_id='{$level_id}',expired_date='{$expired_date}' " . $sqlPass . " WHERE id='{$id}' ";
                break;
            case 'reseller':
                $sql = "UPDATE user SET reseller_id='{$reseller_id}',login='{$login}',real_name='{$real_name}',user_access='{$user_access}',address='{$address}',phone='{$phone}',email='{$email}',description='{$description}',expired_date='{$expired_date}' " . $sqlPass . "  WHERE id='{$id}' ";
                break;
            case 'user':
        ...
        if ($result) {
            ...
            $response['code'] = 'ERROR';
            $response['msg'] = "Edit Data User Error :" . mysql_error();
        }
        break;
    case 'delete':
        switch ($user_level) {
